...
function Jenkins(runner, options) {
  ...
  var fd, currentSuite;
  var jsonResults = {};
  options = options && options.reporterOptions || {}; // Default options
  ...
  options.jenkins_reporter_test_dir = process.env.JENKINS_REPORTER_TEST_DIR || options.jenkins_reporter_test_dir || 'test'; // From http://stackoverflow.com/a/961504 modified for JavaScript
  ...
  function removeInvalidXmlChars(str) {
    ...
    str = str.replace(/([^\ud800-\udbff])[\udc00-\udfff]|^[\udc00-\udfff]/g, '$1'); // Remove other characters that are not valid for XML documents
    ...
    tests.forEach(function (test) {
      ...
      if (test.state == "failed") {
        ...
        if (options.screenshots) {
          ...
          if (options.screenshots == 'loop') {
            ...
            screenshot = path.join(screenshotDir, imagestring + getClassName(test, currentSuite.suite) + test.title + "." + imagetype);
          }

          testCase.testcase.push({
            'system-out': ['[[ATTACHMENT|' + screenshot + ']]']
          });
        }
      ...
      if (test.logEntries && test.logEntries.length) {
        ...
        test.logEntries.forEach(function (entry) {
          ...
          outstr = removeInvalidXmlChars(outstr); // We need to escape CDATA ending tags inside CDATA
  ...
  runner.on('end', function () {
    ...
    if (fd) {
      fs.writeSync(fd, xml(jsonResults, {
        indent: '  '
      }));
      fs.closeSync(fd);
      fd = null;
    ...
    } catch (e) {// Epilogue is not considered critical, fail silently
    }
  });
  ...
  runner.on('pending', function (test) {
    var fmt = indent() + color('checkmark', '  -') + color('pending', ' %s');
    log(fmt, test.title);
